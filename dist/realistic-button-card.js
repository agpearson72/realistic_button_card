/**
 * Realistic Button Card Templates v1.2.0
  ...
  if (eff && String(variables.twinkly_multi_mode||'').toLowerCase() === 'blend') {
    ...
    if (Array.isArray(pal) && pal.length){
      ...
      for (var j=0;j<n;j++){
        ...
        var pct = 12 + Math.round(j * (48/(Math.max(1,n-1))));
        parts.push(c + ' ' + pct + '%');
      }
...
`
    ...
    realistic_button_weather: {
      ...
      custom_fields: {
        rain: `[[[
  function active(){
    if (variables.weather_rain_enabled) return true;
    const id = (variables.weather_rain_entity||'').trim();
    if (!id) return false;
    const s = states[id]?.state; if (s==null) return false;
    const v = String(s).toLowerCase(); const n = parseFloat(v);
    if (!Number.isNaN(n)) return n > 0;
    return ['on','wet','raining','rain','true','detected'].includes(v);
  }
  if (!active()) return '';
  
  const N   = Math.max(4, Math.min(40, Number(variables.weather_rain_drops ?? 16)));
  const H   = Math.max(4, Math.min(25, Number(variables.weather_puddle_height ?? 12)));
  const SPD = Math.max(0.6, Number(variables.weather_rain_speed ?? 1.1));
  ...
  for (let i=0;i<N;i++){
    ...
    parts.push(\`<span class="rb-ripple" style="--x:\${x}%;--dur:\${d}s;--delay:\${delay}s;"></span>\`);
  }
...
`
      ...
      icon: `[[[
  if (variables.weather_icon) return variables.weather_icon;

  const condId =
    variables.weather_condition_entity ||
    (entity?.entity_id?.startsWith('weather.') ? entity.entity_id : '');

  const condRaw = condId ? String(states[condId]?.state ?? '').toLowerCase() : '';
  const cond = condRaw.replace('_', '-').replace(/\\s+/g, '-');

  const prId = String(variables.weather_precip_entity || '');
  const pr = prId ? Number(states[prId]?.state) : NaN;

  const isNight = !!variables.weather_use_daynight &&
  ...
  const nightMap = {
    ...
    'partly-cloudy': 'mdi:weather-night-partly-cloudy'
  };
  const map = isNight ? nightMap : dayMap;

  if (!cond && !Number.isNaN(pr) && pr > 0) {
    return pr > 8 ? 'mdi:weather-pouring' : 'mdi:weather-rainy';
  }

  if (map[cond]) return map[cond];

  if (cond.includes('clear') && isNight) return 'mdi:weather-night';
  if (cond.includes('clear')) return 'mdi:weather-sunny';
  if (/(partly.?cloud|mostly.?cloud)/.test(cond)) {
  ...
  if (/(lightning|thunder|storm)/.test(cond)) {
    return (!Number.isNaN(pr) && pr > 0) ? 'mdi:weather-lightning-rainy' : 'mdi:weather-lightning';
  }
  ...
  return variables.weather_fallback_icon || 'mdi:weather-partly-cloudy';
]]]
`,
      styles: {
        card: [
          {
            "--rb-rain-color": `[[[
  return variables.weather_rain_color || '#4fc3f7';
]]]
`
          }
        ],
        custom_fields: {
          ...
          glow: [
            {
              opacity: "[[[ return variables.weather_disable_base_glow ? 0 : null ]]]"
            },
            {
              display: "[[[ return variables.weather_disable_base_glow ? \"none\" : null ]]]"
            }
          ],
          under_glow: [
            {
      ...
      extra_styles: `.rb-rain {
  position:absolute; inset:0; border-radius:inherit; pointer-events:none; overflow:hidden;
}
.rb-puddle {
  position:absolute; left:0; right:0; bottom:0;
  height: var(--puddle-h, 12%);
  background:
    radial-gradient(60% 140% at 50% 130%,
      color-mix(in srgb, var(--rb-rain-color,#4fc3f7) 60%, transparent) 0%,
      transparent 75%),
    linear-gradient(to top,
      color-mix(in srgb, var(--rb-rain-color,#4fc3f7) 35%, transparent) 0%,
      transparent 100%);
  filter: blur(.6px);
  z-index:0;
}
.rb-drop {
  position:absolute; left:var(--x); top:-12%;
  width:2px; --drop-h:12%; height:var(--drop-h);
  border-radius:2px; background:var(--rb-rain-color,#4fc3f7); opacity:.9;
  animation: rb-fall-top var(--dur,1.1s) linear var(--delay,0s) infinite;
  z-index:2;
}
.rb-ripple {
  position:absolute;
  top: calc(100% - var(--puddle-h,12%));
  left: var(--x);
  transform: translate(-50%, 0);
  width: 0; height: 0;
  border: 2px solid color-mix(in srgb, var(--rb-rain-color,#4fc3f7) 70%, transparent);
  border-radius: 999px;
  opacity: 0;
  filter: drop-shadow(0 0 2px color-mix(in srgb, var(--rb-rain-color,#4fc3f7) 60%, transparent));
...
`
  ...
  if (full) {
    ...
    var rad = flame(col.charAt(0)==='#' && col.length===7 ? col : col, '88');
    return rad + ', ' + lin;
  } else {
...
`
      ...
      extra_styles: `@keyframes rb_flame_move {
